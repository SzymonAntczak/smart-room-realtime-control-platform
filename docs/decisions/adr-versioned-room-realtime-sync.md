...
Later changes use named, validated `device.updated` messages. Each carries the
changed device projection and its newest-first, bounded history of ten event
summaries. Every delta carries its
previous and new revision. A client must apply only the next contiguous
