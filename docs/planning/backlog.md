...
| Rating | Meaning                                                   |
| ------ | --------------------------------------------------------- |
| `A`    | Exceptional difficulty or directly decision-blocked work. |
| `B`    | High-risk cross-boundary, reliability or recovery work.   |
| `C`    | Complex standard feature work.                            |
| `D`    | Moderate, bounded work with limited risk.                 |
| `E`    | Small, local and deterministic work.                      |
| `F`    | Banal mechanical work.                                    |

- **Expected outcome:** the simulator route provides bounded durable history,
  understandable realtime state, diagnostics and a reproducible local demo.
- **Sources reviewed:** Stage 4 storage ADR; system overview; control loop;
  devices; events and commands; reliability and testing; realtime and SSE ADRs.
- **Assumptions:** existing code is evidence to audit against each Done condition,
  not automatic completion of an unchecked item.
...
- **Sources / boundaries:** Stage 4 ADR; JSON Schema ADR; `shared` contracts,
  backend BFF and frontend HTTP/SSE boundaries.
- **Depends on:** none.
- **Difficulty:** `B`.
- **Difficulty rationale:** shared contract, identity, ordering and cursor-scope risk.
- **Acceptance criteria:** bounded ordered feeds; original sample identities;
  half-open trends; schema-valid typed cursor errors.
- **Non-goals:** storage queries, BFF endpoints and rendering.
- **Story verification:** shared contract tests with positive and negative bounds.

| Subtask      | Purpose / primary boundary                                                      | Depends on                               | Difficulty | Verification / done when                                                                 |
| ------------ | ------------------------------------------------------------------------------- | ---------------------------------------- | ---------- | ---------------------------------------------------------------------------------------- |
| `ST-4-01-01` | Complete recent-event, durable-diagnostics and cursor-page schemas in `shared`. | none                                     | `B`        | Contract tests reject unordered, oversized, dangling and timestamp-inconsistent entries. |
...
- **Sources / boundaries:** Stage 4 ADR; events and commands; backend storage
  port and SQLite adapter.
- **Depends on:** `DS-4-01`.
- **Difficulty:** `B`.
- **Difficulty rationale:** time, transactions, deduplication and physical purge interact.
- **Acceptance criteria:** exact time/count retention, final-record dedup horizon,
  pinned bounds and safe post-expiry purge.
...
- **Sources / boundaries:** Stage 4 ADR; JSON Schema ADR; storage port and BFF.
- **Depends on:** `DS-4-01`, `DS-4-02`.
- **Difficulty:** `B`.
- **Difficulty rationale:** cursor and generation checks protect a cross-boundary read path.
- **Acceptance criteria:** schema-valid endpoints, pinned bounds, immutable scope,
  generation rejection and durable-read `503`.
...
- **Sources / boundaries:** Stage 4 ADR; room realtime synchronization ADR; SSE
  ADR; backend coordinator/BFF and frontend realtime/history clients.
- **Depends on:** `DS-4-03`.
- **Difficulty:** `A`.
- **Difficulty rationale:** revision ordering, batches and recovery are time-sensitive.
...
- **Sources / boundaries:** devices; availability/freshness ADR; Stage 4 ADR;
  device definitions, projector and simulator runtime.
- **Depends on:** none.
- **Difficulty:** `C`.
- **Difficulty rationale:** injected time crosses configuration and projection.
- **Acceptance criteria:** interval per capability, `3 × expectedIntervalMs`,
  manual reading through normal flow and independent availability.
...
- **Sources / boundaries:** Stage 4 ADR; events and commands; frontend feed UI
  and mocked-BFF browser boundary.
- **Depends on:** `ST-4-01-01`, `ST-4-04-01`.
- **Difficulty:** `C`.
- **Difficulty rationale:** main risk is truthful presentation and exclusions.
- **Acceptance criteria:** permanent bounded feed with device/time/command context
...
- **Difficulty rationale:** several independent durability axes must remain
  visible.
- **Acceptance criteria:** persistent status banner, volatile labels, degraded
  warning, recovering block and no phantom command on a racing `503`.
...
- **Sources / boundaries:** Stage 4 ADR; roadmap; reliability/testing; frontend
  HTTP history and SSE clients plus temperature UI.
- **Depends on:** `DS-4-03`, `DS-4-04`, `DS-4-05`.
- **Difficulty:** `B`.
- **Difficulty rationale:** pagination, live merge, reconnect and accessibility
  meet here.
- **Acceptance criteria:** accessible entry point, chart/table equivalence,
  bounded baseline, live updates and reconnect refetch with honest labels.
...
- **Sources / boundaries:** Stage 4 ADR; events and commands; simulator,
  processor, storage, diagnostics API and structured logs.
- **Depends on:** `ST-4-01-01`, `DS-4-02`.
- **Difficulty:** `B`.
- **Difficulty rationale:** the proof crosses simulator, persistence, API and
  logs.
...
- **Human decisions required:** none for Stage 4.
